...
Furthermore, it is interesting to note the effect of sPlot on any combinatorial background we may have from incorrect associations of final-state pions. Since each event may contain multiple reconstructed topologies (combos), we may have events where combining the incorrect pion pair yields a topology which passes through reconstruction. Since we select the combo with the best $\chi^2_\nu$ for each event, we bias the resulting events towards ones which contain the correct pion combinations. However, we still may have a background from events where the wrong combination happens to have a better $\chi^2_\nu$ as well as events where only one or neither combination comes from a real kaon. In \Cref{fig:hypo-ks12-v-ks21-data-pz-masscut-chisqdof-3.0-mesons} we plot the invariant masses of the incorrect combinations against each other. By examining the signal Monte Carlo in \Cref{fig:hypo-ks12-v-ks21-sigmc-pz-masscut-chisqdof-3.0-mesons}, we expect to see some peak at the kaon mass in both distributions, but we do not expect the additional structure of \Cref{fig:hypo-ks12-v-ks21-data-pz-masscut-chisqdof-3.0-mesons}. However, after sPlot weights are applied (see \Cref{fig:hypo-ks12-v-ks21-data-pz-masscut-chisqdof-3.0-mesons-free-2}), this structure vanishes, and the distribution matches that of the signal Monte Carlo.

\begin{figure}
    \centering
    \begin{subfigure}{0.45\textwidth}
      \includesvg[width=\linewidth]{ext/analysis/plots/hypo_ks12_v_ks21_data_pz_masscut_chisqdof_3.0_mesons_None_None.svg}
    ...
    \hfill
    \begin{subfigure}{0.45\textwidth}
      \includesvg[width=\linewidth]{ext/analysis/plots/hypo_ks12_v_ks21_sigmc_pz_masscut_chisqdof_3.0_mesons_None_None.svg}
    ...
    \begin{subfigure}{0.8\textwidth}
      \includesvg[width=\linewidth]{ext/analysis/plots/hypo_ks12_v_ks21_data_pz_masscut_chisqdof_3.0_mesons_free_2.svg}
